...
function wrap<T extends ((...args: any[]) => K) | (() => K), K>(fn: T, name?: string): T {
  return ((...a: Parameters<T>) => showActualValueOnError<K>(name ?? fn.name, a, fn(...a))) as any;
}
...
export const anyPercentage = wrap(anyToolkit.anyPercentage, "anyPercentage");
export const anyInteger = wrap(anyToolkit.anyInteger, "anyInteger");
export const anyPositiveInteger = wrap(anyToolkit.anyPositiveInteger, "anyPositiveInteger");
export const anyNegativeInteger = wrap(anyToolkit.anyNegativeInteger, "anyNegativeInteger");
